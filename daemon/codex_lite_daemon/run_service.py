...
class EventHub:
    ...
    def __init__(self, db: Database | None = None) -> None:
        ...
        self._lifecycle_listener: Callable[[str], None] | None = None

    def set_lifecycle_listener(self, listener: Callable[[str], None] | None) -> None:
    ...
    async def publish(self, run_id: str, event: str, data: dict) -> None:
        ...
        if event in {"done", "error"}:
            await self.flush()
        if event in {"status", "done", "error"} and self._lifecycle_listener is not None:
            self._lifecycle_listener(run_id)
...
class RunService:
    ...
    def list_run_diagnostics(self) -> list[dict]:
        ...
        for run_id, active in self.active_runs.items():
            ...
            chat = self.db.fetchone("SELECT project_id FROM chats WHERE id = ?", (item.get("chatId"),))
            item["projectId"] = str(chat.get("project_id") or "") if chat else ""
