...
class AppServerClient:
    ...
    def __init__(self, config: Config, codex_runner: CodexRunner, model_provider: str | None = None) -> None:
        ...
        self._subscribers: dict[asyncio.Queue[AppServerNotification], tuple[str, str | None] | None] = {}
    ...
    def subscribe_queue(self, thread_id: str | None = None, turn_id: str | None = None) -> asyncio.Queue[AppServerNotification]:
        queue: asyncio.Queue[AppServerNotification] = asyncio.Queue()
        self._subscribers[queue] = (thread_id, turn_id) if thread_id is not None else None
        return queue

    def bind_queue(self, queue: asyncio.Queue[AppServerNotification], thread_id: str, turn_id: str | None = None) -> None:
        if queue not in self._subscribers:
            raise RuntimeError("app-server notification queue is not subscribed")
        self._subscribers[queue] = (thread_id, turn_id)

    def unsubscribe_queue(self, queue: asyncio.Queue[AppServerNotification]) -> None:
        self._subscribers.pop(queue, None)

    async def _dispatch_notification(self, notification: AppServerNotification) -> None:
        for queue, route in list(self._subscribers.items()):
            if route is None or notification.method == "app_server/exited" or _notification_matches_route(notification, route):
    ...
    async def _read_stdout(self) -> None:
        ...
        await self._dispatch_notification(exit_notification)
...
def _notification_matches_route(notification: AppServerNotification, route: tuple[str, str | None]) -> bool:
    thread_id, turn_id = route
