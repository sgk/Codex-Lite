...
class TranscriptImportService:
    ...
    def _read_transcript_session(self, path: Path, allow_missing_cwd: bool = False) -> TranscriptSession | None:
        ...
        if project_path is None and allow_missing_cwd:
            project_path = self._historical_candidate_path(cwd)
    ...
    def list_messages(self, project_path: str, session_id: str, chat_id: str, transcript_path: str | None = None, include_internal: bool = False) -> list[dict]:
        transcript = self._validated_transcript_path(transcript_path, project_path, session_id, include_internal=include_internal, require_project_match=False) if transcript_path else None
    ...
    def find_transcript_path(self, project_path: str, session_id: str, transcript_path: str | None = None, include_internal: bool = False) -> Path | None:
        if transcript_path:
            transcript = self._validated_transcript_path(
                transcript_path,
                project_path,
                session_id,
    ...
    def _validated_transcript_path(
        ...
        if not any(resolved.is_relative_to(root) for root in allowed_roots):
            return None
        session = self._read_transcript_session(resolved, allow_missing_cwd=not require_project_match)
    ...
    def _historical_candidate_path(self, value: str) -> Path | None:
        if "\x00" in value:
            return None
        path_value = _windows_path_to_wsl(value)
        if "\\" in path_value:
