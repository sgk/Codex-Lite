...
class AppServerThreadService:
    ...
    async def list_messages(self, project_id: str, chat_id: str) -> list[dict]:
        ...
        all_local_messages = _recover_truncated_local_conclusions(transcript_messages, all_local_messages)
...
class AppServerRunService:
    ...
    async def start_message_run(self, project_id: str, chat_id: str, content: str, attachments: list[dict[str, Any]] | None = None) -> dict:
        ...
        async with self._runtime_reconcile_lock:
            ...
            if chat_id in self._starting_chats or any(run.chat_id == chat_id and run.status == "running" for run in self.runs.values()):
                raise AppError("run_already_active_in_chat", "Another run is already active in this chat.", 409)
            self._starting_chats.add(chat_id)
        try:
    ...
    async def _start_message_run_reserved(
        ...
        try:
            ...
            raise
        notification_queue: asyncio.Queue[AppServerNotification] | None = None
        try:
            ...
            notification_queue = app_server.subscribe_queue(candidate_thread_ids[0])
            response = None
            thread_id = candidate_thread_ids[0]
            last_thread_not_found: AppError | None = None
            for candidate_thread_id in candidate_thread_ids:
                try:
                    app_server.bind_queue(notification_queue, candidate_thread_id)
        ...
        if not turn_id or notification_queue is None:
            if notification_queue is not None:
                app_server.unsubscribe_queue(notification_queue)
            await _release_run_lease(app_server)
        ...
        app_server.bind_queue(notification_queue, thread_id, turn_id)
    ...
    def list_active_runs(self) -> list[dict[str, Any]]:
        ...
        for run_id, run in self.runs.items():
            if run.status != "running":
                continue
            chat = self.chats.db.fetchone("SELECT project_id FROM chats WHERE id = ?", (run.chat_id,))
            result.append(
                _run_out(run_id, run)
                | {
                    "eventSequence": self.events.current_sequence(run_id),
                    "projectId": str(chat.get("project_id") or "") if chat else "",
                }
            )
        return result
    ...
    async def reconcile_chat_runtime(self, project_id: str, chat_id: str) -> None:
        async with self._runtime_reconcile_lock:
            ...
            for provider_thread in self.chats.list_provider_threads(project_id, chat_id):
                ...
                if not bool(getattr(app_server, "is_running", False)):
                    continue
                notification_queue = app_server.subscribe_queue(thread_id)
                ...
                try:
                    ...
                    if now - last_state_reconcile >= RUN_STATE_RECONCILE_SECONDS:
                        ...
                        if status_type == "idle" and not run.replacing_idle_turn:
                            # thread/read can report idle after app-server has
                            # queued the final deltas and turn/completed, but
                            # before this watcher has consumed them.
                            await asyncio.sleep(IDLE_RECONCILE_NOTIFICATION_GRACE_SECONDS)
        ...
        messages.append(
            {
                ...
                "activityKind": "reasoning" if _is_reasoning_delta_method(str(state.get("method") or "")) else "work",
            }
        )
...
def _recover_truncated_local_conclusions(transcript_messages: list[dict], local_messages: list[dict]) -> list[dict]:
    transcript_conclusions = [
        message
        for message in transcript_messages
        if str(message.get("role") or "").lower() == "assistant"
        and str(message.get("kind") or "") == "conclusion"
        and str(message.get("content") or "")
    ]
    if not transcript_conclusions:
    ...
    for local in local_messages:
        ...
        if (
            str(local.get("role") or "").lower() != "assistant"
            or str(local.get("kind") or "") != "conclusion"
            or not local.get("runId")
            or not local_content
        ):
            recovered.append(local)
            continue

        local_timestamp = _message_timestamp(local)
        candidates = [
            transcript
            for transcript in transcript_conclusions
            if len(str(transcript.get("content") or "")) > len(local_content)
            and str(transcript.get("content") or "").startswith(local_content)
            and abs((_message_timestamp(transcript) - local_timestamp).total_seconds()) <= 10
        ]
        if not candidates:
            recovered.append(local)
            continue
        ...
        best = min(
            ...
            key=lambda transcript: (
                abs((_message_timestamp(transcript) - local_timestamp).total_seconds()),
                -len(str(transcript.get("content") or "")),
            ),
        )
        recovered.append({**local, "content": str(best.get("content") or "")})
    return recovered
