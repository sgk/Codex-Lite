...
from codex_lite_daemon.remote_gateway import _remote_attachments, _remote_history
...
from codex_lite_daemon.runner.base import RunnerEvent
...
@pytest.mark.asyncio
async def test_app_server_routes_notifications_to_only_the_matching_run(linux_tmp_path: Path) -> None:
    cfg = make_test_config(linux_tmp_path)
    client = AppServerClient(cfg, CodexRunner(cfg))
    stdout = asyncio.StreamReader()
    for thread_id, turn_id in (("thread_1", "turn_1"), ("thread_2", "turn_2")):
        stdout.feed_data(
            (
                json.dumps(
                    {
                        "method": "item/started",
                        "params": {"threadId": thread_id, "turnId": turn_id, "item": {"id": f"item_{turn_id}"}},
                    }
                )
                + "\n"
            ).encode("utf-8")
    ...
    stdout.feed_eof()
    client._process = SimpleNamespace(stdout=stdout)  # type: ignore[assignment]
    first = client.subscribe_queue("thread_1", "turn_1")
    second = client.subscribe_queue("thread_2", "turn_2")

    await client._read_stdout()

    assert (await first.get()).params["threadId"] == "thread_1"
    assert (await first.get()).method == "app_server/exited"
    assert first.empty()
    assert (await second.get()).params["threadId"] == "thread_2"
    assert (await second.get()).method == "app_server/exited"
    assert second.empty()
...
def test_deepseek_app_server_command_uses_responses_provider(linux_tmp_path: Path) -> None:
    ...
    catalog_path = cfg.app_data_dir / "deepseek-models.json"
    ...
    vision = catalog["models"][2]
    assert vision["input_modalities"] == ["text", "image"]
    assert vision["supports_image_detail_original"] is False
...
def test_model_list_adds_configured_deepseek_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["reasoningEffortsByModel"]["deepseek-v4-pro"] == ["low", "high", "max"]
    assert result["reasoningEffortsByModel"]["deepseek-v4-flash-vision-exp"] == ["low", "high", "max"]
...
def test_recover_truncated_local_conclusion_from_nearby_transcript() -> None:
    transcript_messages = [
        {
            "id": "assistant-transcript",
            "role": "assistant",
            "kind": "conclusion",
            "content": "回答の全文です。続きを含みます。",
            "createdAt": "2026-08-15T14:39:49.982Z",
        }
    ]
    local_messages = [
        {
            ...
            "runId": "run-1",
            "content": "回答の全文です。続",
            "createdAt": "2026-08-15T14:39:51Z",
        }
    ]

    recovered = _recover_truncated_local_conclusions(transcript_messages, local_messages)

    assert recovered[0]["id"] == "assistant-local"
    assert recovered[0]["content"] == "回答の全文です。続きを含みます。"


def test_does_not_replace_local_conclusion_with_unrelated_transcript() -> None:
    transcript_messages = [
        {
            "id": "assistant-transcript",
            "role": "assistant",
            "kind": "conclusion",
            "content": "別の回答です。",
            "createdAt": "2026-08-15T14:39:50Z",
        }
    ]
    local_messages = [
        {
    ...
    assert recovered == local_messages
...
@pytest.mark.asyncio
async def test_event_hub_notifies_catalog_only_for_run_lifecycle() -> None:
    hub = EventHub()
    changed_run_ids: list[str] = []
    hub.set_lifecycle_listener(changed_run_ids.append)

    await hub.publish("run_1", "progress", {"summary": "thinking"})
    await hub.publish("run_1", "status", {"status": "running"})
    await hub.publish("run_1", "output", {"text": "partial"})
    await hub.publish("run_1", "done", {"status": "succeeded"})

    assert changed_run_ids == ["run_1", "run_1"]
...
async def test_event_hub_replays_persisted_events_after_recreation(linux_tmp_path: Path) -> None:
    ...
    terminal_persistence: list[int] = []
    first_hub.set_lifecycle_listener(lambda _run_id: terminal_persistence.append(
        int((db.fetchone("SELECT COUNT(*) AS count FROM run_events WHERE run_id = 'run_1' AND event = 'done'") or {}).get("count") or 0)
    ))
    ...
    assert terminal_persistence[-1] == 1
...
def test_chat_restart_flag_is_persisted_until_cleared(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    cfg = make_test_config(linux_tmp_path)
    db = Database(cfg.database_path)
    ...
    chat = chats.create_chat(project["id"], "restart")

    assert chats.mark_resume_on_start([chat["id"], chat["id"]]) == 1
    pending = chats.list_resume_on_start()
    assert pending == [
        {
            "chat_id": chat["id"],
            "project_id": project["id"],
            "resume_requested_at": pending[0]["resume_requested_at"],
        }
    ]

    chats.clear_resume_on_start(chat["id"])
    assert chats.list_resume_on_start() == []
    db.close()


@pytest.mark.asyncio
async def test_restart_flag_sends_continue_only_once(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    app = create_app(make_test_config(linux_tmp_path))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        project = (await client.post("/projects", json={"path": str(project_dir)})).json()
        chat = (await client.post(f"/projects/{project['id']}/chats", json={"title": "restart"})).json()
        app.state.db.execute(
            "UPDATE chats SET resume_on_start = 1, resume_requested_at = ? WHERE id = ?",
            (utc_now(), chat["id"]),
        )

        resumed = await client.post("/runs/resume-after-app-restart")
        repeated = await client.post("/runs/resume-after-app-restart")
        messages = (await client.get(f"/projects/{project['id']}/chats/{chat['id']}/messages")).json()

        assert resumed.status_code == 200
        assert len(resumed.json()["resumed"]) == 1
        assert resumed.json()["pendingCount"] == 0
        assert repeated.json()["resumed"] == []
        assert [message["content"] for message in messages if message["kind"] == "instruction"] == ["つづけて"]


@pytest.mark.asyncio
async def test_active_chat_is_marked_before_app_restart(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    app = create_app(make_test_config(linux_tmp_path))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        started = await client.post(
            ...
            json={"content": "long work"},
        )
        prepared = await client.post("/runs/prepare-app-restart")

        assert started.status_code == 200
        assert prepared.json() == {"markedCount": 1}
        flagged = app.state.db.fetchone("SELECT resume_on_start FROM chats WHERE id = ?", (chat["id"],))
        assert flagged is not None
        assert flagged["resume_on_start"] == 1
...
@pytest.mark.asyncio
async def test_automation_run_wakes_remote_catalog_after_run_registration(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    app = create_app(make_test_config(linux_tmp_path))
    ...
    class BlockingRunner:
        async def run(self, _prompt: str, _project_path: str):
            runner_started.set()
            await release_runner.wait()
            yield RunnerEvent("stdout", "done")

    app.state.runs.runner = BlockingRunner()
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        project = (await client.post("/projects", json={"path": str(project_dir)})).json()
        chat = (await client.post(f"/projects/{project['id']}/chats", json={"title": "daily"})).json()
        automation = (
        ...
        revision_before = (await client.get("/remote/v1/catalog-revision")).json()["revision"]

        response = await client.post(f"/projects/{project['id']}/chats/{chat['id']}/automations/{automation['id']}/run")
        assert response.status_code == 200
        run_id = response.json()["run"]["runId"]
        await asyncio.wait_for(runner_started.wait(), timeout=1)
        revision_after = (await client.get("/remote/v1/catalog-revision")).json()["revision"]
        catalog = (await client.get("/remote/v1/catalog?includeHistory=false")).json()

        assert revision_after > revision_before
        remote_chat = catalog["projects"][0]["chats"][0]
        assert remote_chat["status"] == "running"
        assert remote_chat["activeRunId"] == run_id

        active_task = app.state.runs.active_runs[run_id].task
        release_runner.set()
        await active_task
...
@pytest.mark.asyncio
async def test_remote_gateway_exposes_sanitized_catalog_and_idempotent_send(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "remote-project"
    project_dir.mkdir()
    app = create_app(make_test_config(linux_tmp_path))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        project = (await client.post("/projects", json={"path": str(project_dir), "name": "Remote"})).json()
        chat = (await client.post(f"/projects/{project['id']}/chats", json={"title": "Outside"})).json()

        catalog = (await client.get("/remote/v1/catalog")).json()
        assert catalog["revision"] == 2
        del catalog["revision"]
        assert catalog == {
                    ...
                    "chats": [
                        {
                            ...
                            "title": "Outside",
                                ...
                                "historyRevision": f"{chat['updatedAt']}|0||0||0",
                                "history": [],
                        }
                    ],
                }
        ...
        assert str(project_dir) not in json.dumps(catalog)

        metadata_catalog = (await client.get("/remote/v1/catalog?includeHistory=false")).json()
        assert "history" not in metadata_catalog["projects"][0]["chats"][0]
        history = await client.get(f"/remote/v1/projects/{project['id']}/chats/{chat['id']}/history")
        assert history.status_code == 200
        assert history.json() == {"history": []}

        initial_history_revision = metadata_catalog["projects"][0]["chats"][0]["historyRevision"]
        app.state.db.execute(
            "INSERT INTO runs(id, chat_id, status, started_at, finished_at, revision) VALUES ('history-revision-run', ?, 'succeeded', ?, ?, 1)",
            (chat["id"], chat["updatedAt"], chat["updatedAt"]),
        )
        app.state.db.execute(
            "INSERT INTO run_events(run_id, sequence, event, data_json, created_at) VALUES ('history-revision-run', 1, 'progress', '{}', ?)",
            (chat["updatedAt"],),
        )
        changed_metadata = (await client.get("/remote/v1/catalog?includeHistory=false")).json()
        assert changed_metadata["projects"][0]["chats"][0]["historyRevision"] != initial_history_revision

        body = {
            "operation": "send_message",
            "payload": {"projectId": project["id"], "chatId": chat["id"], "content": "remote hello"},
        }
        first = await client.post("/remote/v1/tasks/task-1/execute", json=body)
        repeated = await client.post("/remote/v1/tasks/task-1/execute", json=body)
        assert first.status_code == 200
        assert repeated.status_code == 200
        assert repeated.json() == first.json()
        current_revision = (await client.get("/remote/v1/catalog-revision")).json()["revision"]
        assert current_revision > 2

        forbidden = await client.post(
            "/remote/v1/tasks/task-2/execute",
            json={"operation": "read_file", "payload": {"path": "secret"}},
        )
        assert forbidden.status_code == 400
        assert forbidden.json()["error"]["code"] == "remote_operation_not_allowed"


@pytest.mark.asyncio
async def test_remote_catalog_exposes_active_desktop_run_without_progress_body(linux_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    project_dir = linux_tmp_path / "active-remote-project"
    project_dir.mkdir()
    app = create_app(make_test_config(linux_tmp_path))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        project = (await client.post("/projects", json={"path": str(project_dir), "name": "Active"})).json()
        chat = (await client.post(f"/projects/{project['id']}/chats", json={"title": "Running"})).json()
        app.state.remote_gateway.use_app_server = True
        monkeypatch.setattr(app.state.remote_gateway.app_runs, "list_active_runs", lambda: [{
            "id": "run-desktop-1",
            "chatId": chat["id"],
            "status": "running",
            "eventSequence": 17,
    ...
    remote_chat = catalog["projects"][0]["chats"][0]
    assert remote_chat["status"] == "running"
    assert remote_chat["activeRunId"] == "run-desktop-1"
    assert remote_chat["activeRunEventSequence"] == 17
    assert "reasoning" not in remote_chat


@pytest.mark.asyncio
async def test_remote_catalog_does_not_import_unregistered_project_candidates(linux_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    app = create_app(make_test_config(linux_tmp_path))
    remote_gateway = app.state.remote_gateway

    def fail_if_called() -> None:
        raise AssertionError("Remote catalog must not register unregistered transcript projects")

    monkeypatch.setattr(remote_gateway.transcript_import, "import_project_candidates", fail_if_called)
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get("/remote/v1/catalog")

    assert response.status_code == 200


@pytest.mark.asyncio
async def test_remote_catalog_preserves_desktop_order_and_reports_sync_priority(linux_tmp_path: Path) -> None:
    app = create_app(make_test_config(linux_tmp_path))
    remote_gateway = app.state.remote_gateway
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        projects = []
        ...
        for name in ("one", "two", "three"):
            project_dir = linux_tmp_path / name
            project_dir.mkdir()
            project = (await client.post("/projects", json={"path": str(project_dir), "name": name})).json()
            projects.append(project)
            chats.append((await client.post(f"/projects/{project['id']}/chats", json={"title": f"{name}-first"})).json())
            second_chat = (await client.post(f"/projects/{project['id']}/chats", json={"title": f"{name}-second"})).json()
            if name == "one":
                first_project_second_chat_id = second_chat["id"]

        selected_project = projects[1]
        selected_chat = chats[1]
        priority = await client.post(
            "/remote/v1/sync-priority",
            json={
                "selectedProjectId": selected_project["id"],
                "selectedChatId": selected_chat["id"],
                "projectOrder": [projects[2]["id"], projects[0]["id"], projects[1]["id"]],
                "chatOrder": {projects[0]["id"]: [first_project_second_chat_id, chats[0]["id"]]},
            },
        )
        assert priority.status_code == 200
        first_revision = priority.json()["revision"]

        unchanged_priority = await client.post(
            "/remote/v1/sync-priority",
            json={
                "selectedProjectId": selected_project["id"],
                "selectedChatId": selected_chat["id"],
        ...
        assert unchanged_priority.status_code == 200
        assert unchanged_priority.json()["revision"] == first_revision
        assert (await client.get("/remote/v1/catalog-revision")).json() == {"revision": first_revision}

        selection_only = await client.post(
            "/remote/v1/sync-priority",
            json={
        ...
        assert selection_only.status_code == 200
        assert selection_only.json()["revision"] == first_revision

        wait_for_change = asyncio.create_task(remote_gateway.wait_catalog_revision(first_revision, timeout_seconds=1))
        await asyncio.sleep(0)
        message = await client.post(
            f"/projects/{projects[0]['id']}/chats/{chats[0]['id']}/messages",
            json={"content": "sync this message"},
        )
        assert message.status_code == 200
        assert await wait_for_change > first_revision

        catalog = (await client.get("/remote/v1/catalog")).json()
        assert [project["id"] for project in catalog["projects"]] == [
            projects[2]["id"], projects[0]["id"], projects[1]["id"],
        ]
        assert catalog["projects"][1]["chats"][0]["id"] == first_project_second_chat_id
        assert catalog["selectedProjectId"] == projects[0]["id"]
        assert catalog["selectedChatId"] == chats[0]["id"]


def test_remote_history_is_complete_and_redacted() -> None:
    history = _remote_history(
        [
            {"role": "system", "content": "internal"},
            {"role": "assistant", "kind": "analysis", "content": "private reasoning"},
            {"id": "u1", "role": "user", "content": "read /home/sgk/project", "createdAt": "2026-08-28T00:00:00Z"},
            {"id": "s1", "role": "status", "kind": "status", "content": "コマンドを実行しました", "activityDetails": "npm test\npassed", "activityKind": "work", "runId": "run-1", "createdAt": "2026-08-28T00:00:00.500Z"},
            {"id": "a1", "role": "assistant", "kind": "conclusion", "content": "Authorization: Bearer abcdefghijklmnop", "createdAt": "2026-08-28T00:00:01Z"},
        ],
        "/home/sgk/project",
    )
    assert [item["id"] for item in history] == ["u1", "s1", "a1"]
    assert history[0]["content"] == "read <local-project>"
    assert history[1]["activityDetails"] == "npm test\npassed"
    assert history[1]["activityKind"] == "work"
    assert history[2]["content"] == "Authorization=<redacted>"

    many = _remote_history([
        {
            "id": f"message-{index}",
            "role": "status",
            "kind": "status",
            "content": "作業",
            "activityDetails": "x" * 13_000,
            "createdAt": f"2026-08-28T00:00:{index % 60:02d}Z",
        }
...
def test_remote_attachments_accept_only_agent_temporary_directory(linux_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: linux_tmp_path))
    attachment_root = linux_tmp_path / ".local" / "share" / "codex-lite" / "remote-attachments" / "task-1"
    attachment_root.mkdir(parents=True)
    image = attachment_root / "image-1.png"
    image.write_bytes(b"png")
    document = attachment_root / "attachment-2.pdf"
    document.write_bytes(b"pdf")

    assert _remote_attachments({"attachments": [
        {"path": str(image), "name": "screen.png", "kind": "image"},
        {"path": str(document), "name": "report.pdf", "kind": "file"},
    ]}) == [
        {"path": str(image), "name": "screen.png", "kind": "image"},
        {"path": str(document), "name": "report.pdf", "kind": "file"},
    ]
    with pytest.raises(AppError, match="path is invalid"):
        _remote_attachments({"attachments": [{"path": str(linux_tmp_path / "outside.png"), "name": "outside.png"}]})
...
def test_input_history_returns_latest_user_instructions_without_loading_full_history(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    cfg = make_test_config(linux_tmp_path)
    db = Database(cfg.database_path)
    ...
    project = projects.create_project(str(project_dir))
    chat = chats.create_chat(project["id"], "history")
    for index in range(105):
        messages.insert_message(chat["id"], "user", f"instruction-{index:03d}", kind="instruction")
        messages.insert_message(chat["id"], "assistant", f"work-{index:03d}", kind="work")
    messages.insert_message(chat["id"], "assistant", "conclusion", kind="conclusion")

    history = messages.list_input_history(project["id"], chat["id"], 100)

    assert len(history) == 100
    assert history[0] == "instruction-005"
    assert history[-1] == "instruction-104"
    db.close()
...
@pytest.mark.asyncio
async def test_app_server_messages_use_saved_transcript_path_after_project_relocation_without_scanning(linux_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    cfg = make_test_config(linux_tmp_path)
    sessions = cfg.codex_home / "sessions"
    ...
    destination_parent = linux_tmp_path / "moved"
    destination_parent.mkdir()
    projects.relocate_project(project["id"], str(destination_parent), "renamed-project")
...
async def test_app_server_run_allows_parallel_different_chats(linux_tmp_path: Path) -> None:
    ...
    assert {run["projectId"] for run in runs.list_active_runs()} == {project_id}
    db.close()
...
async def test_app_server_idle_state_recovers_missing_completed_notification(linux_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(app_services, "IDLE_RECONCILE_NOTIFICATION_GRACE_SECONDS", 0.005)
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    cfg = make_test_config(linux_tmp_path)
    db = Database(cfg.database_path)
...
@pytest.mark.asyncio
async def test_app_server_idle_state_drains_queued_final_notifications_before_recovery(linux_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(app_services, "REASONING_PROGRESS_BATCH_SECONDS", 0.005)
    monkeypatch.setattr(app_services, "RUN_STATE_RECONCILE_SECONDS", 0.01)
    monkeypatch.setattr(app_services, "IDLE_RECONCILE_NOTIFICATION_GRACE_SECONDS", 0.005)
    project_dir = linux_tmp_path / "project"
    ...
    messages = MessageService(db, chats)
    transcripts = TranscriptImportService(cfg, projects, chats)
    app_server = IdleReadWithPendingFinalAppServer()
    threads = AppServerThreadService(projects, chats, messages, transcripts, app_server, make_runtime_settings())  # type: ignore[arg-type]
    runs = AppServerRunService(projects, threads, messages, app_server, max_concurrent_runs=1, settings=make_runtime_settings())  # type: ignore[arg-type]
    ...
    stored = messages.list_messages(project_id, chat_id)
    assert runs.get_run(result["runId"])["terminalReason"] == "turn_completed:completed"
    assert [(message["role"], message["content"]) for message in stored if message["role"] == "assistant"] == [
        ("assistant", "末尾まで届いた回答です。")
    ]
    db.close()
...
@pytest.mark.asyncio
async def test_app_server_does_not_adopt_turn_while_same_chat_is_starting(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    cfg = make_test_config(linux_tmp_path)
    db = Database(cfg.database_path)
    ...
    messages = MessageService(db, chats)
    transcripts = TranscriptImportService(cfg, projects, chats)
    app_server = DelayedTurnStartAppServer()
    threads = AppServerThreadService(projects, chats, messages, transcripts, app_server, make_runtime_settings())  # type: ignore[arg-type]
    runs = AppServerRunService(projects, threads, messages, app_server, max_concurrent_runs=2, settings=make_runtime_settings(), db=db)  # type: ignore[arg-type]

    project_id = projects.create_project(str(project_dir))["id"]
    chat_id = chats.upsert_chat_index(project_id, "thread_1", "existing", "thread_1", utc_now(), utc_now())["id"]
    chats.upsert_provider_thread(project_id, chat_id, "openai", "thread_1", history_initialized=True)

    start_task = asyncio.create_task(runs.start_message_run(project_id, chat_id, "hello"))
    await asyncio.wait_for(app_server.turn_start_entered.wait(), timeout=1)
    await runs.reconcile_chat_runtime(project_id, chat_id)

    assert runs.list_active_runs() == []
    assert not any(method == "thread/turns/list" for method, _ in app_server.requests)

    app_server.allow_turn_start.set()
    result = await asyncio.wait_for(start_task, timeout=1)
    active = runs.list_active_runs()
    assert len(active) == 1
    assert active[0]["id"] == result["runId"]
    assert active[0]["adopted"] is False
    db.close()
...
@pytest.mark.asyncio
async def test_app_server_agent_message_output_keeps_item_phase_and_stores_final_only(linux_tmp_path: Path) -> None:
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    cfg = make_test_config(linux_tmp_path)
    db = Database(cfg.database_path)
    ...
    await queue.put(AppServerNotification("item/started", {**common, "item": {"id": "comment_1", "type": "agentMessage", "text": "", "phase": "commentary"}}))
    ...
    await queue.put(AppServerNotification("item/started", {**common, "item": {"id": "final_1", "type": "agentMessage", "text": "", "phase": "final_answer"}}))
...
@pytest.mark.asyncio
async def test_get_run_conclusion_returns_only_the_saved_conclusion(linux_tmp_path: Path) -> None:
    cfg = make_test_config(linux_tmp_path)
    project_dir = linux_tmp_path / "project"
    project_dir.mkdir()
    db = Database(cfg.database_path)
    ...
    db.execute("INSERT INTO runs(id, chat_id, status) VALUES ('run_1', 'chat_1', 'succeeded')")
    db.execute("INSERT INTO runs(id, chat_id, status) VALUES ('run_2', 'chat_1', 'succeeded')")
    db.execute(
        "INSERT INTO messages(id, chat_id, role, content, run_id, created_at, kind) VALUES ('msg_work', 'chat_1', 'assistant', 'working', 'run_1', ?, 'work')",
        (now,),
    )
    db.execute(
        "INSERT INTO messages(id, chat_id, role, content, run_id, created_at, kind) VALUES ('msg_final', 'chat_1', 'assistant', 'finished', 'run_1', ?, 'conclusion')",
        (now,),
    )
    db.close()
    ...
    app = create_app(cfg)
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        conclusion = await client.get("/runs/run_1/conclusion")
        missing_conclusion = await client.get("/runs/run_2/conclusion")
        missing_run = await client.get("/runs/not-found/conclusion")

    assert conclusion.status_code == 200
    assert conclusion.json()["id"] == "msg_final"
    assert conclusion.json()["content"] == "finished"
    assert missing_conclusion.status_code == 200
    assert missing_conclusion.json() is None
    assert missing_run.status_code == 404

@pytest.mark.asyncio
async def test_relocate_project_directory_and_keep_display_name(linux_tmp_path: Path) -> None:
    source = linux_tmp_path / "source-project"
    source.mkdir()
    (source / "README.md").write_text("moved", encoding="utf-8")
    destination_parent = linux_tmp_path / "projects"
    destination_parent.mkdir()
    app = create_app(make_test_config(linux_tmp_path))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        project = (await client.post("/projects", json={"path": str(source), "name": "Display name"})).json()
        response = await client.post(
            f"/projects/{project['id']}/relocate",
            json={"parentPath": str(destination_parent), "directoryName": "renamed-project"},
        )

        assert response.status_code == 200
        relocated = response.json()
        target = destination_parent / "renamed-project"
        assert relocated["name"] == "Display name"
        assert relocated["path"] == str(target.resolve())
        assert not source.exists()
        assert (target / "README.md").read_text(encoding="utf-8") == "moved"
        assert (await client.get(f"/projects/{project['id']}")).json()["path"] == str(target.resolve())


@pytest.mark.asyncio
async def test_relocate_project_rejects_existing_destination_and_invalid_name(linux_tmp_path: Path) -> None:
    source = linux_tmp_path / "source-project"
    source.mkdir()
    destination_parent = linux_tmp_path / "projects"
    destination_parent.mkdir()
    (destination_parent / "existing").mkdir()
    app = create_app(make_test_config(linux_tmp_path))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        project = (await client.post("/projects", json={"path": str(source)})).json()
        existing = await client.post(
            f"/projects/{project['id']}/relocate",
            json={"parentPath": str(destination_parent), "directoryName": "existing"},
        )
        invalid = await client.post(
            f"/projects/{project['id']}/relocate",
            json={"parentPath": str(destination_parent), "directoryName": "../escape"},
        )

        assert existing.status_code == 409
        assert existing.json()["error"]["code"] == "project_destination_exists"
        assert invalid.status_code == 400
        assert invalid.json()["error"]["code"] == "project_directory_name_invalid"
...
class TurnStartAppServer:
    def __init__(self) -> None:
        ...
        self.subscribers = []
        self.subscriber_routes: dict[asyncio.Queue, tuple[str, str | None] | None] = {}

    async def ensure_started(self) -> None:
        return None

    def subscribe_queue(self, thread_id: str | None = None, turn_id: str | None = None) -> asyncio.Queue:
        queue: asyncio.Queue = asyncio.Queue()
        self.subscribers.append(queue)
        self.subscriber_routes[queue] = (thread_id, turn_id) if thread_id is not None else None
        return queue

    def bind_queue(self, queue: asyncio.Queue, thread_id: str, turn_id: str | None = None) -> None:
        self.subscriber_routes[queue] = (thread_id, turn_id)

    def unsubscribe_queue(self, queue: asyncio.Queue) -> None:
        if queue in self.subscribers:
            self.subscribers.remove(queue)
        self.subscriber_routes.pop(queue, None)
...
class IdleReadWithPendingFinalAppServer(IdleReadAppServer):
    def __init__(self) -> None:
        super().__init__()
        self.queued_final = False

    async def request(self, method: str, params: dict) -> dict:
        if method == "thread/read" and not self.queued_final:
            self.queued_final = True
            queue = self.subscribers[-1]
            common = {"threadId": "thread_1", "turnId": "turn_1"}
            queue.put_nowait(
                AppServerNotification(
                    "item/started",
                    {**common, "item": {"id": "final_1", "type": "agentMessage", "text": "", "phase": "final_answer"}},
                )
            )
            queue.put_nowait(
                AppServerNotification(
                    "item/agentMessage/delta",
                    {**common, "itemId": "final_1", "delta": "末尾まで届いた回答です。"},
                )
            )
            queue.put_nowait(
                AppServerNotification(
...
class DelayedTurnStartAppServer(ActiveTurnInspectAppServer):
    def __init__(self) -> None:
        super().__init__()
