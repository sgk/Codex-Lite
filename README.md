...
リモート操作の初期実装、Firebase設定、Remote Agentの起動方法は [remote/README.md](remote/README.md) にまとめています。
...
Codex Liteを再起動すると、モデル選択肢に `deepseek-v4-flash` と `deepseek-v4-pro` が表示されます。どちらも同じAPIキー、接続先、DeepSeekアカウント残高を使用し、チャットごとに切り替えられます。ファイルがない、キーが空、`sk-` で始まらない、または空白を含む場合はDeepSeekモデルを候補へ追加しません。APIキーの値は診断画面やログへ表示しません。
...
開発中は次の補助ターゲットを使います。先にビルドを完了し、成功後の配置処理は独立したWindowsプロセスへ引き渡します。出力一式を実行用ディレクトリの隣へ準備してから、実行中のCodex Liteへ更新のための正常終了を通知し、実行用ディレクトリを短時間で入れ替えます。その後、デスクトップの `Codex Lite` ショートカットを実行用ディレクトリへ向け、最新ビルドを起動します。Codex Lite自身から実行した場合も、独立した配置プロセスとの名前付きパイプ経由で自分自身を正常終了します。
