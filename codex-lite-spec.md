...
チャット本文またはMarkdownプレビュー内のプロジェクト内ファイルリンクをクリックしたときは、ファイルタブへ切り替え、途中のディレクトリを展開して対象ファイルを選択する。タブ切替とディレクトリ展開後はツリー項目のUI生成を待ってから次の項目を選択し、タブだけが開いて対象ファイルが未選択になる競合を避ける。
