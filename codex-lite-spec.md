...
送信後はSSEでイベントを受け取り、作業中の説明、ツール実行、回答の差分、完了・失敗を逐次反映する。チャット本文およびMarkdownファイルのプレビューでは、空行で区切られた通常段落の後に1行分の余白を表示し、`~~文字列~~` を取り消し線として表示し、`![代替テキスト](画像パス)` を画像として埋め込む。フェンス付きコードブロックにはコード本文だけをクリップボードへ保存するコピーボタンを表示する。チャット内の相対画像パスはプロジェクトルート、Markdownファイル内ではそのファイルの親ディレクトリを基準にする。画像はproject root外やシンボリックリンクを読まないdaemonの読み取り専用APIを通して取得し、読み込み失敗時と外部URLでは代替テキストをクリック可能なリンクとして表示する。外部画像URLを画面表示だけで自動取得しない。実行中は停止できる。また、同じチャットの実行中に入力した指示は `turn/steer` により追加指示として送る。画面上は実行中でもapp-server側ですでにターンが完了していた場合、入力内容を失敗させず同じスレッドの次ターンとして開始する。停止はapp-serverの `turn/interrupt` を応答待ちのリクエストとして送り、スレッド状態が `idle` になるまで追加指示と次ターン開始を無効にし、停止前後のターンIDを混在させない。app-serverが終了した場合は待機中のRunを失敗として終了し、完了通知を取り逃した場合も定期的なスレッド状態確認で `idle` へ収束させ、Runを無期限に実行中のまま残さない。`thread/read` による状態照合が一時的に失敗してもapp-serverのターンを停止せず、Run監視を継続して照合失敗を診断へ記録する。
