...
OpenAIとDeepSeekの切り替えは、Codex app-serverのプロバイダー設定をアプリ全体で切り替える。DeepSeekを選択した場合はDeepSeekのResponses API設定とモデルカタログを使い、プロバイダーが変わるときは次回のapp-server要求で再起動する。実行中のRunを切断する切り替えは行わず、APIキーの内容はログ・診断・画面へ出さない。
