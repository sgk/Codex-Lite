...
assistantメッセージのライブ表示では、`item/started` と `item/agentMessage/delta` の項目IDおよび `phase` を保持する。`commentary` は「作業内容」、`final_answer` は「結論」として別メッセージに分け、途中報告を最終回答へ連結しない。phaseを通知しない旧モデル・提供元では従来互換の表示を行う。既存JSONLの書き換えや過去表示の移行は行わない。
