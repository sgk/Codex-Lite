...
public sealed class DaemonClient
{
    ...
    public Task SetRemoteSyncPriorityAsync(string? selectedProjectId, string? selectedChatId, IEnumerable<string> projectOrder, IReadOnlyDictionary<string, IEnumerable<string>> chatOrder, CancellationToken cancellationToken = default) =>
        PostAsync<object>("/remote/v1/sync-priority", new { selectedProjectId, selectedChatId, projectOrder, chatOrder }, cancellationToken);
    ...
    public Task<ProjectDto?> RelocateProjectAsync(string projectId, string parentPath, string directoryName, CancellationToken cancellationToken = default) =>
        PostAsync<ProjectDto>($"/projects/{projectId}/relocate", new { parentPath, directoryName }, cancellationToken);
    ...
    public Task<List<string>?> ListInputHistoryAsync(string projectId, string chatId, CancellationToken cancellationToken = default) =>
        _http.GetFromJsonAsync<List<string>>($"/projects/{projectId}/chats/{chatId}/input-history", JsonOptions, cancellationToken);
    ...
    public Task<AppRestartPreparationDto?> PrepareRunsForAppRestartAsync(CancellationToken cancellationToken = default) =>
        PostAsync<AppRestartPreparationDto>("/runs/prepare-app-restart", new { }, cancellationToken);

    public Task<AppRestartResumeDto?> ResumeRunsAfterAppRestartAsync(CancellationToken cancellationToken = default) =>
        PostAsync<AppRestartResumeDto>("/runs/resume-after-app-restart", new { }, cancellationToken);
    ...
    public Task<MessageDto?> GetRunConclusionAsync(string runId, CancellationToken cancellationToken = default) =>
        _http.GetFromJsonAsync<MessageDto>($"/runs/{runId}/conclusion", JsonOptions, cancellationToken);

    public Task<RemoteCatalogRevisionDto?> GetRemoteCatalogRevisionAsync(CancellationToken cancellationToken = default) =>
        _http.GetFromJsonAsync<RemoteCatalogRevisionDto>("/remote/v1/catalog-revision", JsonOptions, cancellationToken);

    public Task<RemoteCatalogRevisionDto?> WaitRemoteCatalogRevisionAsync(long afterRevision, CancellationToken cancellationToken = default) =>
        _http.GetFromJsonAsync<RemoteCatalogRevisionDto>($"/remote/v1/catalog-revision/wait?after={afterRevision.ToString(CultureInfo.InvariantCulture)}", JsonOptions, cancellationToken);
