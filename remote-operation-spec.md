...
現在実装済みの通常動作は `codex-lite-spec.md` を正本とする。リモート操作の初期実装は `remote/`、導入手順は `remote/README.md` を正本とし、本書では採用済みの方針と未実装の検討事項を区別する。Slackを操作UIとして使う案の詳細は `slack-remote-spec.md` に残し、本書で定義する共通の配送・実行機構に対する将来のUI adapterとして扱う。
...
- catalog同期では、対象PCのデスクトップクライアントで選択中のチャット、選択中のプロジェクト、プロジェクトツリー順をこの順に優先する。
...
既存チャットの表示を可能にするため、Remote Agentは検証済みのローカル履歴からユーザー／assistantの表示用メッセージを導出し、チャット文書へ上限付きで同期する。JSONL全体、ローカル絶対パス、認証情報、ツール呼び出し、内部推論は同期しない。
...
初期実装では所有者をパスに含め、Security Rulesとクエリの所有権条件を一致させる。
...
```text
users/{uid}/devices/{deviceId}/projects/{projectId}/chats/{chatId}
```
...
初期実装では任意機能として採用した。暗号鍵を設定しない場合はHTTPSとFirestore Rulesだけを使用し、設定した場合は本文ペイロードを暗号化する。鍵の恒久保存と端末ごとの高度な失効方式は引き続き未決定である。
...
初期UIとしてWeb PWAを `remote/web` に実装した。別PCのCodex Liteは、既存daemonへ直接接続するクライアントではなく、Remote Coreを利用する別の操作側クライアントとして扱う。

## 13. 初期実装範囲と進捗

1. 1ユーザー、1対象PCの手動設定: 実装済み。自動ペアリングは未実装
2. リモート操作の手動ON/OFF: Agentプロセスの起動・停止として実装。Windows UIスイッチは未実装
3. 対象PCのonline/offline表示: 実装済み
4. プロジェクトとチャットの一覧: 実装済み
5. 既存チャットへの指示と新規チャット: 実装済み
6. Firestoreタスクのclaimとローカルdaemonへの配送: 実装済み
7. 実行状態、途中経過、最終回答: 実装済み。連続する回答deltaは最大0.75秒単位で集約し、低レベル進捗の追加集約は今後調整
8. 追加指示と停止: 実装済み
9. 承認と拒否: 実装済み
10. 再接続と冪等性: daemon側task ID記録を実装。実行中イベントの完全な再開検証は未完了
11. 簡易共通鍵暗号化: 任意機能として実装。鍵更新UIは未実装

## 14. 未決事項

- Remote Agentは初期版では `remote/agent` の独立プロセスとした。将来Windowsクライアントへ統合するか。
- Web PWAはFirebase Hosting用設定を実装した。別の静的ホスティングも許可するか。
- 初期版は操作側とRemote AgentがFirestoreへ直接読み書きする。Cloud Run functionsへ移す管理処理の範囲。
